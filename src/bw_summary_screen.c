...
static void RefreshBattleMovesPage(void);
...
static const u8 sText_ViewIVEVs[]                           = _("View IV/EV");
...
void ShowPokemonSummaryScreen_BW(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ...
    if (mode == SUMMARY_MODE_RELEARNER_BATTLE)
        ...
        sMonSummaryScreen->currPageIndex = BW_SUMMARY_SHOW_CONTEST_MOVES ? PSS_PAGE_CONTEST_MOVES : PSS_PAGE_BATTLE_MOVES;
    else
...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    case 1:
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            sum->helperMoves[i] = GetMonData(mon, MON_DATA_HELPER_MOVE1+i);
    ...
    default:
        ...
        if (P_SUMMARY_SCREEN_MOVE_RELEARNER)
        {
            u16 levelUpMovesBuffer[MAX_LEVEL_UP_MOVES];
            sMonSummaryScreen->relearnableMovesNum = GetLevelUpMovesBySpecies(GetMonData(mon, MON_DATA_SPECIES), levelUpMovesBuffer);
        }
        else
        {
...
// Recentered positions for dots 1-3 when the contest page (dot 4) is hidden.
// Shifted +1 tile column right from the originals so the 3 remaining dots
// center on the same midpoint the original 4-dot row spanned.
#define TILEMAP_PAGE_DOT_1_TILE_1_RECENTERED   0x000E
#define TILEMAP_PAGE_DOT_1_TILE_2_RECENTERED   0x002E
#define TILEMAP_PAGE_DOT_2_TILE_1_RECENTERED   0x0010
#define TILEMAP_PAGE_DOT_2_TILE_2_RECENTERED   0x0030
#define TILEMAP_PAGE_DOT_3_TILE_1_RECENTERED   0x0012
...
static void HideContestPageDots(void)
{
    ...
    for (page = 0; page < PSS_PAGE_COUNT; page++)
    {
        ...
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_1_TILE_1] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_1_TILE_2] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_2_TILE_1] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_2_TILE_2] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_3_TILE_1] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[page][TILEMAP_PAGE_DOT_3_TILE_2] = TILE_BLACK_SQUARE;
...
static void LimitEggSummaryPageDisplay(void)
{
    ...
    if (!BW_SUMMARY_SHOW_CONTEST_MOVES)
    {
        sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO][TILEMAP_PAGE_DOT_2_TILE_1_RECENTERED] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO][TILEMAP_PAGE_DOT_2_TILE_2_RECENTERED] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO][TILEMAP_PAGE_DOT_3_TILE_1_RECENTERED] = TILE_BLACK_SQUARE;
        sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO][TILEMAP_PAGE_DOT_3_TILE_2_RECENTERED] = TILE_BLACK_SQUARE;
    }
    ScheduleBgCopyTilemapToVram(2);
}
...
static void RestoreSummaryPageDisplay(void)
{
    ...
    else
    {
        ...
        sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO][TILEMAP_PAGE_DOT_3_TILE_2_RECENTERED] = TILE_INACTIVE_SQUARE_BOTTOM;
    }
    ScheduleBgCopyTilemapToVram(2);
}
...
static void SwitchToMoveSelection(u8 taskId)
{
    ...
    if (IsMoveFusionUnlocked() && !gMain.inBattle && !sMonSummaryScreen->isBoxMon && !sMonSummaryScreen->lockMovesFlag)
        PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_SWITCH,
                          sMonSummaryScreen->summary.helperMoves[sMonSummaryScreen->firstMoveIndex] == MOVE_NONE ? COMPOUND_STRING("{SELECT_BUTTON} FUSE") : COMPOUND_STRING("{SELECT_BUTTON} UNFUSE"),
                          0, 1, 0, 1);
...
static void Task_HandleInput_MoveSelect(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        else if (JOY_NEW(SELECT_BUTTON) && IsMoveFusionUnlocked() && !gMain.inBattle && !sMonSummaryScreen->isBoxMon && !sMonSummaryScreen->lockMovesFlag && sMonSummaryScreen->firstMoveIndex < MAX_MON_MOVES)
        {
            u32 slot = sMonSummaryScreen->firstMoveIndex;
            if (sMonSummaryScreen->summary.helperMoves[slot] != MOVE_NONE)
            {
                ClearMonMoveHelper(&sMonSummaryScreen->monList.mons[sMonSummaryScreen->curMonIndex], slot);
                sMonSummaryScreen->summary.helperMoves[slot] = MOVE_NONE;
                FillWindowPixelBuffer(PSS_LABEL_WINDOW_PROMPT_SWITCH, PIXEL_FILL(0));
                PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_SWITCH, COMPOUND_STRING("{SELECT_BUTTON} FUSE"), 0, 1, 0, 1);
                RefreshBattleMovesPage();
                PlaySE(SE_SELECT);
            }
            else
            {
                ...
                gRelearnMode = RELEARN_MODE_MOVE_FUSION;
                StopPokemonAnimations();
                PlaySE(SE_SELECT);
                BeginCloseSummaryScreen(taskId);
            }
...
static void ChangeSelectedMove(s16 *taskData, s8 direction, u8 *moveIndexPtr)
{
    ...
    // Update the selected slot before redrawing.  Fused move details use the
    // selected slot to resolve its helper, so drawing first would incorrectly
    // show only the main move's description after moving the cursor away and
    // back.
    *moveIndexPtr = newMoveIndex;
    PrintMoveDetails(move);

    FillWindowPixelBuffer(PSS_LABEL_WINDOW_PROMPT_SWITCH, PIXEL_FILL(0));
    if (newMoveIndex < MAX_MON_MOVES && IsMoveFusionUnlocked() && !gMain.inBattle && !sMonSummaryScreen->isBoxMon && !sMonSummaryScreen->lockMovesFlag)
        PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_SWITCH,
                          sMonSummaryScreen->summary.helperMoves[newMoveIndex] == MOVE_NONE ? COMPOUND_STRING("{SELECT_BUTTON} FUSE") : COMPOUND_STRING("{SELECT_BUTTON} UNFUSE"),
                          0, 1, 0, 1);
...
static void SwapMonMoves(struct Pokemon *mon, u8 moveIndex1, u8 moveIndex2)
{
    ...
    u16 helper1 = summary->helperMoves[moveIndex1];
    u16 helper2 = summary->helperMoves[moveIndex2];
    ...
    SetMonData(mon, MON_DATA_HELPER_MOVE1 + moveIndex1, &helper2);
    SetMonData(mon, MON_DATA_HELPER_MOVE1 + moveIndex2, &helper1);
    ...
    summary->helperMoves[moveIndex1] = helper2;
    summary->helperMoves[moveIndex2] = helper1;
...
static void SwapBoxMonMoves(struct BoxPokemon *mon, u8 moveIndex1, u8 moveIndex2)
{
    ...
    SetBoxMonData(mon, MON_DATA_HELPER_MOVE1 + moveIndex1, &helper2);
    SetBoxMonData(mon, MON_DATA_HELPER_MOVE1 + moveIndex2, &helper1);
...
static void PrintPageNamesAndStats(void)
{
    ...
    if (BW_SUMMARY_IV_EV_DISPLAY != BW_IV_EV_HIDDEN)
    {
        ...
        {
            stringXPos = GetStringRightAlignXOffset(FONT_NORMAL, sText_ViewIVEVs, skillsLabelWidth);
            iconXPos = stringXPos - 16;
            if (iconXPos < 0)
                iconXPos = 0;
            PrintAOrBButtonIcon(PSS_LABEL_WINDOW_PROMPT_IVS, FALSE, iconXPos);
            PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_IVS, sText_ViewIVEVs, stringXPos, 1, 0, 1);
...
{
    u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);
    PrintTextOnWindowWithFont(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_ABILITY), gAbilitiesInfo[ability].description, 4, 15, -1, 0, FONT_SMALL);
}
...
static void CycleMonAbility(void)
{
    ...
    if (sMonSummaryScreen->isBoxMon)
        boxMon = &sMonSummaryScreen->monList.boxMons[sMonSummaryScreen->curMonIndex];
    else
        boxMon = &sMonSummaryScreen->monList.mons[sMonSummaryScreen->curMonIndex].box;
...
static void BufferIvEvPair(u8 *dst, u32 iv, u32 ev, u32 strId)
{
    static const u8 sTextNeutral[] = _("{COLOR}{01}");
    u8 ivText[4];
    u8 evText[5];

    ConvertIntToDecimalStringN(ivText, iv, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(evText, ev, STR_CONV_MODE_LEADING_ZEROS, 3);
    StringCopy(dst, sTextNeutral);
    StringAppend(dst, ivText);
    StringAppend(dst, COMPOUND_STRING("/"));
    StringAppend(dst, evText);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(strId, dst);
}
...
static void BufferAndPrintStats_HandleState(u8 mode)
{
    ...
    {
        BufferIvEvPair(maxHPString, sMonSummaryScreen->summary.ivHp, sMonSummaryScreen->summary.evHp, 0);
        DynamicPlaceholderTextUtil_Reset();
        ...
        DynamicPlaceholderTextUtil_Reset();
        BufferIvEvPair(gStringVar1, sMonSummaryScreen->summary.ivAtk, sMonSummaryScreen->summary.evAtk, 0);
        BufferIvEvPair(gStringVar2, sMonSummaryScreen->summary.ivDef, sMonSummaryScreen->summary.evDef, 1);
        BufferIvEvPair(gStringVar3, sMonSummaryScreen->summary.ivSpatk, sMonSummaryScreen->summary.evSpatk, 2);
        BufferIvEvPair(gStringVar4, sMonSummaryScreen->summary.ivSpdef, sMonSummaryScreen->summary.evSpdef, 3);
        BufferIvEvPair(sStringVar5, sMonSummaryScreen->summary.ivSpeed, sMonSummaryScreen->summary.evSpeed, 4);
        PrintNonHPStats();
    }
...
static void RefreshBattleMovesPage(void)
{
    u8 moveNamesWindowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_NAMES_PP);
    u8 descriptionWindowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESCRIPTION);

    // Cancel queued printers first; otherwise an old fused-name printer can
    // finish after the clear and put stale glyphs back into the window.
    DeactivateSingleTextPrinter(moveNamesWindowId, WINDOW_TEXT_PRINTER);
    DeactivateSingleTextPrinter(descriptionWindowId, WINDOW_TEXT_PRINTER);

    // All four move rows share one window. Clear it before drawing shorter
    // unfused names so glyphs from the old fused name cannot remain behind.
    FillWindowPixelBuffer(moveNamesWindowId, PIXEL_FILL(0));
    FillWindowPixelBuffer(descriptionWindowId, PIXEL_FILL(0));
    PrintBattleMoves();
    CopyWindowToVram(moveNamesWindowId, COPYWIN_GFX);
    CopyWindowToVram(descriptionWindowId, COPYWIN_GFX);
}
...
static void PrintMoveNameAndPP(u8 moveIndex)
{
    ...
    if (move != 0)
    {
        if (summary->helperMoves[moveIndex] != MOVE_NONE)
        {
            GetFusedMoveName(move, summary->helperMoves[moveIndex], gStringVar3);
            PrintTextOnWindowToFitPx_WithFont(moveNameWindowId, gStringVar3, 3, moveIndex * 28 + 2, 0, 12, FONT_SMALL, WindowWidthPx(moveNameWindowId) - 3);
        }
        else
        ...
        pp = ResolveMoveFusion(move, summary->helperMoves[moveIndex], &fusion) ? fusion.pp : CalculatePPWithBonus(move, summary->ppBonuses, moveIndex);
...
static void PrintContestMoveDescription(u8 moveSlot)
{
    ...
    {
        windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESCRIPTION);
        FormatTextByWidth(desc, 119, FONT_SMALL, gContestEffects[gMovesInfo[move].contestEffect].description, GetFontAttribute(FONT_SMALL, FONTATTR_LETTER_SPACING));
        PrintTextOnWindowWithFont(windowId, desc, 2, 0, -1, 0, FONT_SMALL);
    }
}
...
static void PrintMoveDetails(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            struct ResolvedMoveFusion fusion;
            bool32 isFused = sMonSummaryScreen->firstMoveIndex < MAX_MON_MOVES
                          && sMonSummaryScreen->summary.moves[sMonSummaryScreen->firstMoveIndex] == move
                          && ResolveMoveFusion(move, sMonSummaryScreen->summary.helperMoves[sMonSummaryScreen->firstMoveIndex], &fusion);
            PrintMovePowerAndAccuracy(move);
            if (isFused)
            {
                ConvertIntToDecimalStringN(gStringVar1, fusion.power, STR_CONV_MODE_RIGHT_ALIGN, 3);
                FillWindowPixelRect(PSS_LABEL_WINDOW_MOVES_POWER_ACC, PIXEL_FILL(0), 2, 4, 19, 12);
                PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, gStringVar1, 2, 4, 0, 0);
                u8 combined[BW_MAX_MOVE_DESCRIPTION_LENGTH];
                const u8 *helperDescription = gMoveFusionInfo[fusion.helperMove].helper.description;
                StringCopy(combined, gMovesInfo[move].effect != EFFECT_PLACEHOLDER ? gMovesInfo[move].description : gNotDoneYetDescription);
                if (helperDescription != NULL)
                {
                    StringAppend(combined, COMPOUND_STRING("\n"));
                    StringAppend(combined, helperDescription);
                }
                for (u32 i = 0; i < fusion.numAdditionalEffects; i++)
                {
                    const struct AdditionalEffect *effect = &fusion.additionalEffects[i];
                    u32 combinedChance;

                    if (!GetMoveFusionCombinedEffectChance(move, effect, &combinedChance))
                        continue;
                    StringAppend(combined, COMPOUND_STRING("\nCombined "));
                    if (effect->moveEffect == MOVE_EFFECT_FLINCH)
                        StringAppend(combined, COMPOUND_STRING("flinch chance: "));
                    else if (effect->moveEffect == MOVE_EFFECT_STAT_MINUS && effect->defense != 0)
                        StringAppend(combined, COMPOUND_STRING("Defense drop chance: "));
                    else
                        StringAppend(combined, COMPOUND_STRING("effect chance: "));
                    ConvertIntToDecimalStringN(gStringVar1, combinedChance, STR_CONV_MODE_LEFT_ALIGN, 3);
                    StringAppend(combined, gStringVar1);
                    StringAppend(combined, COMPOUND_STRING("%."));
                }
                if (fusion.criticalHitStage != 0)
                {
                    StringAppend(combined, COMPOUND_STRING("\nCritical-hit stage: +"));
                    ConvertIntToDecimalStringN(gStringVar1, GetMoveCriticalHitStage(move) + fusion.criticalHitStage, STR_CONV_MODE_LEFT_ALIGN, 2);
                    StringAppend(combined, gStringVar1);
                    StringAppend(combined, COMPOUND_STRING("."));
                }
                FormatTextByWidth(desc, 119, FONT_SMALL, combined, GetFontAttribute(FONT_SMALL, FONTATTR_LETTER_SPACING));
                PrintTextOnWindowWithFont(windowId, desc, 2, 0, -1, 0, FONT_SMALL);
            ...
            if (BW_SUMMARY_AUTO_FORMAT_MOVE_DESCRIPTIONS)
            {
                if (gMovesInfo[move].effect != EFFECT_PLACEHOLDER)
                    FormatTextByWidth(desc, 119, FONT_SMALL, gMovesInfo[move].description, GetFontAttribute(FONT_SMALL, FONTATTR_LETTER_SPACING));
                else
                    FormatTextByWidth(desc, 119, FONT_SMALL, gNotDoneYetDescription, GetFontAttribute(FONT_SMALL, FONTATTR_LETTER_SPACING));
            ...
            {
                if (gMovesInfo[move].effect != EFFECT_PLACEHOLDER)
                    PrintTextOnWindowWithFont(windowId, gMovesInfo[move].description, 2, 0, -1, 0, FONT_SMALL);
                else
                    PrintTextOnWindowWithFont(windowId, gNotDoneYetDescription, 2, 0, -1, 0, FONT_SMALL);
            }
        ...
        else
        {
            ...
            {
                PrintTextOnWindowWithFont(windowId, gContestEffects[gMovesInfo[move].contestEffect].description, 2, 0, -1, 0, FONT_SMALL);
            }
        }
        PutWindowTilemap(windowId);
    }
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state, bool32 isShadow)
{
    ...
    case 1:
        ...
        ApplyPersonalityPaletteVariationByTag(summary->species2, summary->pid);
