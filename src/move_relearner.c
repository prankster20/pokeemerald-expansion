...
static u32 GetRelearnerLevelUpMoves(struct BoxPokemon *mon, u16 *moves)
{
    ...
    if (BoxMonHasNature(mon, NATURE_SCHOLARLY))
    {
...
static bool32 HasRelearnerLevelUpMoves(struct BoxPokemon *boxMon)
{
    ...
    // --- Custom Archetype nature: Prodigious ---
    // Learns moves 1 level early - so the relearner offers them 1 level early too.
    if (BoxMonHasNature(boxMon, NATURE_PRODIGIOUS))
        level += 1;

    // --- Custom Archetype nature: Pack-Rat ---
    if (BoxMonHasNature(boxMon, NATURE_PACK_RAT)
     && !BoxMonKnowsMove(boxMon, MOVE_STOCKPILE))
        return TRUE;

    // --- Custom Archetype nature: Scholarly ---
    if (BoxMonHasNature(boxMon, NATURE_SCHOLARLY))
    {
        enum Species eggSpecies = species;
        while (GetSpeciesPreEvolution(eggSpecies) != SPECIES_NONE)
            eggSpecies = GetSpeciesPreEvolution(eggSpecies);
