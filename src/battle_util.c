...
static bool32 HasAmbientNatureOnField(void)
{
    for (enum BattlerId i = 0; i < gBattlersCount; i++)
    {
        if (IsBattlerAlive(i) && HasNature(i, NATURE_AMBIENT))
            return TRUE;
    }
    return FALSE;
}
...
{
    if (timer == NULL || *timer == 0 || !HasAmbientNatureOnField())
        return;
    ...
    for (enum BattlerId i = 0; i < gBattlersCount; i++)
    {
        if (IsBattlerAlive(i) && HasNature(i, NATURE_AMBIENT))
        {
            SetPendingNaturePopup(i, NATURE_AMBIENT);
            break;
        }
    }
}

static bool32 IncrementTimerIfActive(u16 *timer)
{
    if (*timer > 0)
    {
        (*timer)++;
...
static bool32 IncrementWeatherTimerIfActive(void)
{
...
static bool32 ExtendAmbientTimedFieldEffects(void)
{
    bool32 extended = FALSE;

    extended |= IncrementWeatherTimerIfActive();
    extended |= IncrementTimerIfActive(&gFieldTimers.mudSportTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.waterSportTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.wonderRoomTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.magicRoomTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.trickRoomTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.terrainTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.gravityTimer);
    extended |= IncrementTimerIfActive(&gFieldTimers.fairyLockTimer);

    for (enum BattleSide side = 0; side < NUM_BATTLE_SIDES; side++)
    {
        extended |= IncrementTimerIfActive(&gSideTimers[side].reflectTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].lightscreenTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].mistTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].safeguardTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].auroraVeilTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].tailwindTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].luckyChantTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].damageNonTypesTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].rainbowTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].seaOfFireTimer);
        extended |= IncrementTimerIfActive(&gSideTimers[side].swampTimer);
    }
...
{
    if (!IsBattlerAlive(battler) || !HasNature(battler, NATURE_AMBIENT))
        return FALSE;

    if (!ExtendAmbientTimedFieldEffects())
        return FALSE;

    gBattleScripting.battler = battler;
    gBattleScripting.showNaturePopup = TRUE;
    gBattleScripting.naturePopupId = NATURE_AMBIENT;
...
bool32 TryChangeBattleWeather(enum BattlerId battler, u32 battleWeatherId, enum Ability ability)
{
    ...
    // --- Custom Archetype nature: Ambient ---
    // Timed field effects created while Ambient is active last one extra turn.
    if (HasAmbientNatureOnField() && gBattleStruct->weatherDuration > 0)
    {
        gBattleStruct->weatherDuration++;
        for (enum BattlerId i = 0; i < gBattlersCount; i++)
        {
...
bool32 TryChangeBattleTerrain(enum BattlerId battler, u32 statusFlag)
{
    ...
    if (!(gFieldStatuses & statusFlag))
    {
        ...
        // --- Custom Archetype nature: Ambient ---
        // Timed field effects created while Ambient is active last one extra turn.
        if (HasAmbientNatureOnField() && gFieldTimers.terrainTimer > 0)
        {
...
// pranks / jimh - Custom Archetype nature: Resolute
...
// battlerAtk = the attacker with Persuasive, turns = base duration.
u32 GetPersuasiveInflictedDuration(enum BattlerId battlerAtk, u32 turns)
{
    return turns + HasNature(battlerAtk, NATURE_PERSUASIVE);
}
...
// Shared by ABILITY_MAGICIAN and the Custom Archetype nature: Greedy (steals
// held items after attacking). Returns TRUE if an item was stolen.
static bool32 TryMagicianItemSteal(enum BattlerId battler, enum Move move)
{
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_CURSED_BODY:
             ...
             && RandomPercentage(RNG_CURSED_BODY, 30))
            {
                ...
                else if (!IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
                {
                    gBattleMons[gBattlerAttacker].volatiles.disabledMove = gChosenMove;
                    gBattleMons[gBattlerAttacker].volatiles.disableTimer = GetGullibleVolatileDuration(gBattlerAttacker, B_DISABLE_TIMER);
        ...
        case ABILITY_CUTE_CHARM:
            ...
            {
                u32 immuneNature = GetInfatuationImmuneNature(gBattlerAttacker);
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        // After landing a damaging attack, has a 1/8 chance to infatuate.
        if (!effect
         && HasNature(battler, NATURE_FLIRTY)
         && IsBattlerAlive(gBattlerTarget)
         && IsBattlerTurnDamaged(gBattlerTarget, EXCLUDING_SUBSTITUTES)
         && !gBattleMons[gBattlerTarget].volatiles.infatuation
         && GetBattlerAbility(gBattlerTarget) != ABILITY_OBLIVIOUS
         && !IsAbilityOnSide(gBattlerTarget, ABILITY_AROMA_VEIL)
         && RandomChance(RNG_NATURE_FLIRTY, 1, 8))
        {
            ...
            else
            {
                ...
                gBattleScripting.battler = battler;
                gBattleScripting.showNaturePopup = TRUE;
                gBattleScripting.naturePopupId = NATURE_FLIRTY;
                BattleScriptCall(BattleScript_FlirtyInfatuates);
            }
            effect++;
        }
        break;
    ...
    case ABILITYEFFECT_MOVE_END_FOES_FAINTED:
        ...
        case ABILITY_MAGICIAN:
            effect = TryMagicianItemSteal(battler, move);
            break;
        case ABILITY_MOXIE:
        ...
        // Steals held items after attacking (reuses Magician's mechanic).
        if (!effect && HasNature(battler, NATURE_GREEDY))
        {
            effect = TryMagicianItemSteal(battler, move);
            if (effect)
                SetPendingNaturePopup(battler, NATURE_GREEDY); // pranks / jimh
        }
        // --- Custom Archetype nature: Soft-Hearted ---
        // When knocking out a Pokémon, suffers 1/12 max HP as damage. Timed
        // like Moxie - same caseID, same NumFaintedBattlersByAttacker check.
        if (!effect
         && HasNature(battler, NATURE_SOFT_HEARTED)
        //  && !NoAliveMonsForEitherParty()
         && NumFaintedBattlersByAttacker(battler))
        {
            SetPassiveDamageAmount(battler, GetNonDynamaxMaxHP(battler) / 12);
            gBattleScripting.battler = battler;
            gBattleScripting.showNaturePopup = TRUE;
            gBattleScripting.naturePopupId = NATURE_SOFT_HEARTED;
            BattleScriptCall(BattleScript_SoftHeartedSelfDamage);
            effect = TRUE;
        }
        break;
...
u32 IsAbilityPreventingEscape(enum BattlerId battler)
{
    ...
    if (HasNature(battler, NATURE_FLIGHTY))
        return 0;
...
bool32 CanBattlerEscape(enum BattlerId battler) // no ability check
{
    ...
    // --- Custom Archetype nature: Flighty ---
    // Ignores trapping effects.
    else if (HasNature(battler, NATURE_FLIGHTY))
        return TRUE;
    else if (GetConfig(B_GHOSTS_ESCAPE) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_GHOST))
        return TRUE;
...
// pranks / jimh - Custom Archetype nature: Flighty
// Checks the same conditions CanBattlerEscape/IsAbilityPreventingEscape do,
// minus Flighty's own bypass (and minus the Commander check, which isn't
// really "trapping" in the sense Flighty cares about) - so callers can tell
// whether Flighty's bypass actually mattered, for the switch-out popup.
bool32 WasBattlerActuallyTrapped(enum BattlerId battler)
{
    if (GetConfig(B_GHOSTS_ESCAPE) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_GHOST))
        return FALSE;
...
bool32 HasEnoughHpToEatBerry(enum BattlerId battler, enum Ability ability, u32 hpFraction, enum Item itemId)
{
    ...
    if (hpFraction <= 4 && GetItemPocket(itemId) == POCKET_BERRIES
         ...
         && (IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_GLUTTONY)
          || HasNature(battler, NATURE_VORACIOUS)))
        return TRUE;
...
enum HoldEffect GetBattlerHoldEffect(enum BattlerId battler)
{
    ...
    // Behaves like Klutz: the item remains held, but its battle effect is
    // unavailable. The ignore-ability accessor below deliberately bypasses
    // this too, matching the engine's special Klutz checks.
    if (!gBattleStruct->battlerState[battler].notOnField
     && !gSpecialStatuses[battler].attackerInParty
     && HasNature(battler, NATURE_FINICKY))
...
static bool32 IsMethodicalMoveSlot(enum BattlerId battler, enum Move move, u32 slot)
{
    u32 moveSlot;
    ...
    if (battler == gBattlerAttacker)
        moveSlot = gCurrMovePos;
    else
        moveSlot = GetMoveSlot(gBattleMons[battler].moves, move);

    return moveSlot == slot;
}

static bool32 IsMoveInSpeciesLevelUpLine(enum Species species, enum Move move)
{
...
static inline u32 CalcTerrainBoostedPower(struct DamageContext *ctx, u32 basePower)
{
    ...
    if (HasNature(ctx->battlerAtk, NATURE_APATHETIC))
        return basePower;
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_WEATHER_BALL:
        if (!HasNature(ctx->battlerAtk, NATURE_APATHETIC)
         && GetAttackerWeather(ctx->holdEffects[ctx->battlerAtk], ctx->abilities[ctx->battlerAtk], ctx->weather) & (B_WEATHER_ANY & ~B_WEATHER_STRONG_WINDS))
            basePower *= 2;
        break;
    case EFFECT_PURSUIT:
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case EFFECT_EARTHQUAKE:
        if (ctx->fieldStatuses & STATUS_FIELD_GRASSY_TERRAIN
         && !HasNature(battlerDef, NATURE_APATHETIC)
    ...
    // Deals 10% less move damage.
    if (HasNature(battlerAtk, NATURE_CHILDISH))
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.9));

    // --- Custom Archetype nature: Relentless ---
    // Deals 10% more move damage.
    if (HasNature(battlerAtk, NATURE_RELENTLESS))
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.1));

    // --- Custom Archetype nature: Scholarly ---
    // Boosts moves studied outside the species' level-up line.
    if (HasNature(battlerAtk, NATURE_SCHOLARLY)
     && move != MOVE_NONE
     && move != MOVE_STRUGGLE
    ...
    if (IsMethodicalMoveSlot(battlerAtk, ctx->chosenMove, 0))
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.1));

    // --- Custom Archetype nature: Nostalgic ---
    // The first two move slots are locked in and boosted.
    if (HasNature(battlerAtk, NATURE_NOSTALGIC) && gCurrMovePos < 2)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
    ...
    // If a move fails by any means, grants a 1.5x damage boost on the next turn.
    // Reuses stompingTantrumTimer - it's already a generic "did my last
    // move fail" tracker (set in MoveEndClearBits for any battler/move),
    // not something exclusive to the move Stomping Tantrum.
    if (HasNature(battlerAtk, NATURE_BAD_TEMPERED) && gBattleStruct->battlerState[battlerAtk].stompingTantrumTimer == 1)
    {
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        if (ctx->updateFlags) // pranks / jimh - don't pop up during AI damage prediction
            SetPendingNaturePopup(battlerAtk, NATURE_BAD_TEMPERED);
    }
    ...
    // The first Sound move after switching in deals 30% less damage. This is
    // a "use it up" debuff, not a "must be turn 1" one - it stays armed
    // across however many turns it takes until a Sound move actually gets
    // used, then it's consumed and doesn't apply again until the next switch-in.
    // Gated on ctx->updateFlags so AI damage-prediction passes (which run this
    // same calculation as a dry run) can't consume it without a real move use.
    if (HasNature(battlerAtk, NATURE_TONGUE_TIED)
     && IsSoundMove(move)
     && !gBattleStruct->battlerState[battlerAtk].tongueTiedConsumed)
    {
...
static inline uq4_12_t ApplyOffensiveBadgeBoost(uq4_12_t modifier, enum BattlerId battler, enum Move move)
{
    if (ShouldGetStatBadgeBoost(B_FLAG_BADGE_BOOST_ATTACK, battler) && IsBattleMovePhysical(move))
        modifier = uq4_12_multiply_half_down(modifier, GetBadgeBoostModifier(battler));
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    if (HasNature(battlerAtk, NATURE_FLIGHTY) && moveEffect == EFFECT_HIT_ESCAPE)
        atkStat = atkStat * 110 / 100;

    // --- Custom Archetype nature: Devoted ---
    // Boosts own Atk/SpA by the bond tier percent when a party partner exists.
    if (HasNature(battlerAtk, NATURE_DEVOTED))
    {
        u32 boost = GetDevotedBondData(GetBattlerMon(battlerAtk), NULL);
        if (boost > 0)
            atkStat = atkStat * (100 + boost) / 100;
    }
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (HasNature(ctx->battlerAtk, NATURE_CALCULATING) && (moveEffect != EFFECT_PSYSHOCK && moveEffect != EFFECT_CATACLYSM))
    {
        ...
        spDefStage = gBattleMons[battlerDef].statStages[STAT_SPDEF];

        // Apply stage-ignoring rules to both halves before averaging.
        if (ctx->isCrit)
        {
            if (defStage > DEFAULT_STAT_STAGE)
                defStage = DEFAULT_STAT_STAGE;
            if (spDefStage > DEFAULT_STAT_STAGE)
                spDefStage = DEFAULT_STAT_STAGE;
        }
        if (ctx->abilities[ctx->battlerAtk] == ABILITY_UNAWARE
         || MoveIgnoresDefenseEvasionStages(move))
        {
            defStage = DEFAULT_STAT_STAGE;
            spDefStage = DEFAULT_STAT_STAGE;
        }

        def = def * gStatStageRatios[defStage][0] / gStatStageRatios[defStage][1];
        spDef = spDef * gStatStageRatios[spDefStage][0] / gStatStageRatios[spDefStage][1];
        defStat = (def + spDef) / 2;
    ...
    if (_devotedDefenseBoost != 0)
        defStat = defStat * (100 + _devotedDefenseBoost) / 100;
...
static uq4_12_t GetWeatherDamageModifier(struct DamageContext *ctx)
{
    ...
    if (HasNature(ctx->battlerAtk, NATURE_APATHETIC) || HasNature(ctx->battlerDef, NATURE_APATHETIC))
        return UQ_4_12(1.0);
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    // --- Custom Archetype nature: Phobic ---
    // Takes 10% more damage from super-effective moves.
    if (ctx->typeEffectivenessModifier >= UQ_4_12(2.0)
     && HasNature(ctx->battlerDef, NATURE_PHOBIC))
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.1));
...
static inline s32 DoMoveDamageCalcVars(struct DamageContext *ctx)
{
    ...
    if (ctx->randomFactor)
    {
        u32 damageRollLow = DMG_ROLL_PERCENT_LO;

        // --- Custom Archetype nature: Realistic ---
        // Damage variance is still present, but the roll is always in the
        // upper half of the normal range.
        if (HasNature(ctx->battlerAtk, NATURE_REALISTIC))
            damageRollLow = (DMG_ROLL_PERCENT_HI + DMG_ROLL_PERCENT_LO + 1) / 2;

        dmg *= DMG_ROLL_PERCENT_HI - RandomUniform(RNG_DAMAGE_MODIFIER, 0, DMG_ROLL_PERCENT_HI - damageRollLow);
        dmg /= 100;
    }
...
s32 ApplyModifiersAfterDmgRoll(struct DamageContext *ctx, s32 dmg)
{
    ...
    // --- Custom Archetype nature: Pack-Rat / Hoard Counters ---
    // Each Hoard Counter grants +10% damage, then a real damage calculation
    // spends 1 counter. AI/prediction passes see the boost but do not mutate.
    if (gBattleMons[ctx->battlerAtk].volatiles.stockpileCounter > 0)
    {
        u32 hoardCounters = gBattleMons[ctx->battlerAtk].volatiles.stockpileCounter;
        DAMAGE_APPLY_MODIFIER(PercentToUQ4_12AddOne(hoardCounters * 10));
        if (ctx->updateFlags && !ctx->aiCalc)
            gBattleMons[ctx->battlerAtk].volatiles.stockpileCounter--;
    }
...
static bool32 IsCriticalHit(struct DamageContext *ctx)
{
    ...
    // --- Custom Archetype nature: Diplomatic ---
    // No Pokémon can land critical hits while a Diplomatic mon is on the field.
    // This overrides even moves that always crit (Storm Throw, Frost Breath, etc.)
    // since it checks any active battler, not just the attacker or defender.
    for (u32 _dip = 0; _dip < gBattlersCount; _dip++)
    {
        if (IsBattlerAlive(_dip) && HasNature(_dip, NATURE_DIPLOMATIC))
            return FALSE;
    }
    ...
    // ordinary modern base crit rate, i.e. 1/24. If the attacker actually
    // invests in crit modifiers, Idealistic has to deal with reality.
    else if (HasNature(ctx->battlerDef, NATURE_IDEALISTIC)
          && GetConfig(B_CRIT_CHANCE) != GEN_1
          && GetConfig(B_CRIT_CHANCE) != GEN_2
          && critChance >= 0
          && GetCriticalHitOdds(critChance) == 24)
        isCrit = FALSE;
...
s32 GetAdjustedDamage(struct DamageContext *ctx, s32 damage)
{
    ...
    // If it would be knocked out by move damage before its pending action,
    // let it finish that action, then faint during MoveEnd. This deliberately
    // does not save a battler that has already acted this turn.
    else if (ctx->updateFlags
          && HasNature(ctx->battlerDef, NATURE_INDOMITABLE)
          && gChosenActionByBattler[ctx->battlerDef] == B_ACTION_USE_MOVE
          && GetBattlerTurnOrderNum(ctx->battlerDef) > gCurrentTurnActionNumber)
    {
        enduredHit = TRUE;
        indomitableHit = TRUE;
                              ...
                              5 + (gBattleMons[ctx->battlerDef].friendship * 15 / MAX_FRIENDSHIP)))
    {
        ...
        if (ctx->updateFlags)
        {
            gBattleStruct->moveResultFlags[ctx->battlerDef] |= MOVE_RESULT_FOE_ENDURED_AFFECTIONATE;
            gBattleScripting.battler = ctx->battlerDef;
            gBattleScripting.showNaturePopup = TRUE;
            gBattleScripting.naturePopupId = NATURE_AFFECTIONATE;
        }
    }
    // --- Custom Archetype nature: Noble ---
    // Once per battle, a Noble Pokémon spares a wild opponent at 1 HP.
    else if (ctx->updateFlags
          && HasNature(ctx->battlerAtk, NATURE_NOBLE)
          && IsOnPlayerSide(ctx->battlerAtk)
          && !IsOnPlayerSide(ctx->battlerDef)
          && !(gBattleTypeFlags & BATTLE_TYPE_TRAINER)
          && !GetBattlerPartyState(ctx->battlerAtk)->nobleMercyUsed)
    {
        enduredHit = TRUE;
        GetBattlerPartyState(ctx->battlerAtk)->nobleMercyUsed = TRUE;
        GetBattlerPartyState(ctx->battlerAtk)->nobleMercyPopupPending = TRUE;
    }
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType, bool32 zealousPrimaryType)
{
    uq4_12_t mod = GetTypeModifier(ctx->moveType, defType);

    if (zealousPrimaryType)
        mod = EnhanceZealousPrimaryTypeModifier(mod);
...
static inline void TryNoticeIllusionInTypeEffectiveness(enum Move move, enum Type moveType, enum BattlerId battlerAtk, enum BattlerId battlerDef, uq4_12_t resultingModifier, enum Species illusionSpecies)
{
    ...
    MulByTypeEffectiveness(&ctx, &presumedModifier, GetSpeciesType(illusionSpecies, 0), HasNature(battlerDef, NATURE_ZEALOUS));
    if (GetSpeciesType(illusionSpecies, 1) != GetSpeciesType(illusionSpecies, 0))
        MulByTypeEffectiveness(&ctx, &presumedModifier, GetSpeciesType(illusionSpecies, 1), FALSE);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    MulByTypeEffectiveness(ctx, &modifier, types[0], HasNature(ctx->battlerDef, NATURE_ZEALOUS));
    if (types[1] != types[0])
        MulByTypeEffectiveness(ctx, &modifier, types[1], FALSE);
    if (types[2] != TYPE_MYSTERY && types[2] != types[1] && types[2] != types[0])
        MulByTypeEffectiveness(ctx, &modifier, types[2], FALSE);
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(enum Move move, enum Species speciesDef, enum Ability abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        MulByTypeEffectiveness(&ctx, &modifier, GetSpeciesType(speciesDef, 0), FALSE);
        if (GetSpeciesType(speciesDef, 1) != GetSpeciesType(speciesDef, 0))
            MulByTypeEffectiveness(&ctx, &modifier, GetSpeciesType(speciesDef, 1), FALSE);
...
s32 GetStealthHazardDamage(enum TypeSideHazard hazardType, enum BattlerId battler)
{
    ...
    if (HasNature(battler, NATURE_ZEALOUS))
    {
        ...
        if (types[1] != types[0])
            modifier = uq4_12_multiply(modifier, GetTypeModifier((u8)hazardType, types[1]));

        if (modifier == UQ_4_12(0.0))
            return 0;

        s32 damage = uq4_12_multiply_by_int_half_down(modifier, maxHp) / 8;
        return max(damage, 1);
    }
...
bool32 ShouldGetStatBadgeBoost(u16 badgeFlag, enum BattlerId battler)
{
    ...
    bool32 isAmbitious = (badgeFlag != 0
        && HasNature(battler, NATURE_AMBITIOUS));
...
enum DamageCategory GetBattleMoveCategory(enum Move move)
{
    if (gBattleStruct != NULL)
    {
        ...
        // --- Custom Archetype nature: Old-Fashioned ---
        // Uses the Gen 1-3 type-based Physical/Special split for damaging moves.
        // Fairy is treated as Special (doesn't exist in Gen 3, but matches its
        // "feel" and the player-facing description).
        if (HasNature(gBattlerAttacker, NATURE_OLD_FASHIONED))
...
bool32 CanFling(enum BattlerId battlerAtk, enum Ability abilityAtk)
{
    ...
    if (item == ITEM_NONE
      ...
      || HasNature(battlerAtk, NATURE_FINICKY)
...
{
    gBattleMons[battler].attack = gBattleMons[battler].attack * 100 / 105;
    gBattleMons[battler].defense = gBattleMons[battler].defense * 100 / 105;
    gBattleMons[battler].speed = gBattleMons[battler].speed * 100 / 105;
    gBattleMons[battler].spAttack = gBattleMons[battler].spAttack * 100 / 105;
    gBattleMons[battler].spDefense = gBattleMons[battler].spDefense * 100 / 105;
}
...
u32 CalcSecondaryEffectChance(enum BattlerId battler, enum Ability battlerAbility, const struct AdditionalEffect *additionalEffect)
{
    ...
    // --- Custom Archetype nature: Relentless ---
    // Adds 10 percentage points (e.g. 10% becomes 20%).
    if (additionalEffect->chance != 0 && HasNature(battler, NATURE_RELENTLESS))
        secondaryEffectChance += 10;

    // --- Custom Archetype nature: Methodical ---
    // Moves in slot 2 gain 10 percentage points of effect chance.
    if (additionalEffect->chance != 0
     && IsMethodicalMoveSlot(battler, gCurrentMove, 1))
        secondaryEffectChance += 10;

    // --- Custom Archetype nature: Realistic ---
    // Adds 10 percentage points (e.g. 10% becomes 20%).
    if (additionalEffect->chance != 0 && HasNature(battler, NATURE_REALISTIC))
        secondaryEffectChance += 10;

    return min(secondaryEffectChance, 100);
}
...
bool32 EmergencyExitCanBeTriggered(enum BattlerId battler, enum Ability ability)
{
    ...
    if (ability != ABILITY_EMERGENCY_EXIT && ability != ABILITY_WIMP_OUT
     && !HasNature(battler, NATURE_COWARDLY) && !HasNature(battler, NATURE_PHOBIC))
        return FALSE;

    // --- Custom Archetype natures: Cowardly & Phobic ---
    if (HasNature(battler, NATURE_COWARDLY)) // Wimps Out when HP falls to 25%, instead of the usual 50%.
        hpThresholdCrossed = (gBattleMons[battler].hp + gBattleStruct->moveDamage[battler]) > gBattleMons[battler].maxHP / 4
                           && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 4;
    else if (HasNature(battler, NATURE_PHOBIC)) // Wimps Out when hit by a super-effective move, regardless of HP.
        hpThresholdCrossed = (gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_SUPER_EFFECTIVE) != 0;
    else
...
// Call right before BattleScriptCall(BattleScript_EmergencyExit) so the
// ability popup shows the nature's name instead of the battler's real
// ability, on the off chance it triggered via nature rather than Wimp
// Out/Emergency Exit. Always sets showNaturePopup either way, so a stale
// TRUE from a previous Wimp Out can never leak into some later, unrelated
// ability popup.
void SetNaturePopupForWimpOut(enum BattlerId battler)
{
    if (PokemonHasNature(GetBattlerMon(battler), NATURE_COWARDLY))
    {
        gBattleScripting.showNaturePopup = TRUE;
        gBattleScripting.naturePopupId = NATURE_COWARDLY;
    }
    else
    {
...
// wants to queue up a popup to be shown the next time something consumes
// gBattleScripting.showNaturePopup (e.g. TryShowPendingNaturePopup in
// battle_move_resolution.c, or directly via BattleScript_AbilityPopUpScripting).
// Stores the target battler separately (naturePopupBattler) since
// gBattleScripting.battler itself gets reused/overwritten by lots of other
// things between when this gets queued and when it's actually consumed.
// Only one popup can be pending at a time - if two different battlers'
// natures both want to pop up from the exact same damage instance, only
// the second call here wins. Rare enough in practice not to worry about.
void SetPendingNaturePopup(enum BattlerId battler, u32 nature)
{
    gBattleScripting.naturePopupBattler = battler;
    gBattleScripting.showNaturePopup = TRUE;
...
{
    if (_volatile == VOLATILE_HEAL_BLOCK_TIMER && newValue != 0)
        newValue = GetGullibleVolatileDuration(battler, newValue);
...
bool32 CanMoveSkipAccuracyCalc(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum Move move, enum ResultOption option)
{
    ...
    else if (HasNature(battlerAtk, NATURE_PERFECTIONIST)
          && gBattleMons[battlerDef].volatiles.semiInvulnerable != STATE_COMMANDER
          && !IsSkyDropInvolved(battlerDef, moveEffect))
    {
        effect = TRUE;
...
u32 GetTotalAccuracy(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, enum Ability atkAbility, enum Ability defAbility, enum HoldEffect atkHoldEffect, enum HoldEffect defHoldEffect)
{
    ...
    // --- Custom Archetype nature: Methodical ---
    // Moves in slot 3 have 10% more accuracy.
    if (IsMethodicalMoveSlot(battlerAtk, move, 2))
        calc = (calc * 110) / 100;
...
bool32 DoesOHKOMoveMissTarget(struct BattleCalcValues *cv)
{
    ...
    if (gBattleMons[cv->battlerAtk].volatiles.battlerWithSureHit == cv->battlerDef + 1
          || HasNature(cv->battlerAtk, NATURE_PERFECTIONIST)
    ...
    if (lands == CALC_ACC)
    {
        ...
        if (MoveHasIncreasedAccByTenOnSameType(cv->move) && !IS_BATTLER_OF_TYPE(cv->battlerAtk, GetMoveType(cv->move)))
            odds -= 10;
        if (IsMethodicalMoveSlot(cv->battlerAtk, cv->move, 2))
            odds = odds * 110 / 100;
